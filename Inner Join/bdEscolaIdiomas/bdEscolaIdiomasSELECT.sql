...
SELECT nomeAluno, rgAluno FROM tbAluno
	WHERE dataNascAluno IN (SELECT MAX(dataNascAluno)AS 'Aluno mais novo' FROM tbAluno)

--c--Criar uma consulta que retorne o nome do aluno mais velho;
SELECT nomeAluno, rgAluno FROM tbAluno
	WHERE dataNascAluno IN (SELECT MIN(dataNascAluno)AS 'Aluno mais novo' FROM tbAluno)
